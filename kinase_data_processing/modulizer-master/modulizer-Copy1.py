...
def _subprocess(qin, qout, rxn_list, inco_info, drug_info, args, drug_si, killers):
    while True:
        msg = qin.get()
        if msg == 'STOP':
            break
        if msg[0] in killers:
            ...
            res = _make_retroanalysis(rxn_list, inco_info, drug_info, args, drug_si, msg)
        qout.put((msg, res))
    return None
...
def make_retroanalysis(retros_list, rxn_list, inco_info, drug_info, killers, args, drug_si=None):
    ...
    procs = [Process(target=_subprocess, args=(qin, qout, rxn_list, inco_info, drug_info, args, drug_si, killers)) for _ in range(num_procs)]
