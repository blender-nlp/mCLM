...
pth = '/shared/nas2/chihan3/mCLM/drug_improving_baselines/output/{}/baseline_{}.pkl'

for base in ['chatgpt_gpt-4o','chatgpt_gpt-4o-mini','chatgpt_gpt-5','claude_claude-3-5-haiku-latest','google_genai_gemini-2.5-flash','ldmol']:
    ...
    with open(f'{base}_smiles.txt', 'w') as fw:
        for task in ['ames', 'bbbp', 'cyp3a4', 'dili', 'hia', 'pgp']:
            ...
            with open(pth.format(base, task), 'rb') as f:
            ...
            mCLM_smiles, FDA_smiles, base_smiles = data_to_save['mCLM_smiles'], data_to_save['FDA_smiles'], data_to_save['baseline_smiles']

            if base == 'claude_claude-3-5-haiku-latest': #problem with response
                base_smiles = map(fix_claude, data_to_save['baseline_other_details'])
            ...
            for smi in get_valid(base_smiles2):
                fw.write(smi + '\t' + task +  '\n')
