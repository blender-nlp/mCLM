...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.embed_molecules = GNNMolEncoder(
            **config.molecule_config.to_dict())
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        inputs_embeds = self.embed_chemical_language(input_ids)
    ...
    # mCLM embedding function
    def embed_chemical_language(self, input_ids):
        is_text = input_ids < self.config.vocab_size
        is_molecule = ~is_text
        ...
        mol_inputs_embeds = self.embed_molecules(mol_input_ids)
...
# currently the only modified class
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        # mCLM vocab sizes
        self.mol_vocab_size = config.molecule_config.num_modules
    ...
    def finalized_molecule_embeddings(self):
        all_molecule_ids = torch.arange(
            self.mol_vocab_size,
            dtype=torch.long,
            device=self.device)
        all_molecule_embeddings = self.embed_molecules(all_molecule_ids)
        self.finalized_molecule_embeddings = nn.Linear(
            self.mol_vocab_size,
            self.config.hidden_size,
        )
        self.finalized_molecule_embeddings.weight = nn.Parameter(
            all_molecule_embeddings, requires_grad=False)

    # mCLM logit function
    def mclm_logit_head(
        self, hidden_states, is_training, molecule_ids_to_keep=None
    ):
        text_logits = self.lm_head(hidden_states)
        mol_feature = self.molecule_mlp(hidden_states)
        if is_training:
            assert molecule_ids_to_keep is not None
            negative_set = torch.randperm(
                self.mol_vocab_size
            )[:self.config.negative_sampling_size] + self.vocab_size
            ...
            trained_mol_logits = mol_feature.matmul(
                self.embed_molecules(molecule_ids_trained).transpose(0, 1)
            )
            mol_logits = torch.zeros(
                mol_feature.shape[:-1] + (self.total_vocab_size,),
                device=hidden_states.device,
                dtype=hidden_states.dtype,
            )
            mol_logits[:, molecule_ids_trained] = trained_mol_logits
        else:
            mol_logits = self.finalized_molecule_embeddings(mol_feature)
