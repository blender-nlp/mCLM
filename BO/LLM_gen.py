import pandas as pd
import torch
import torch.nn as nn
import torch.optim as optim
import numpy as np
import random
import re
from rdkit import Chem
from rdkit.Chem import Crippen
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import mean_squared_error, r2_score
import matplotlib.pyplot as plt
from sklearn.decomposition import PCA
from sklearn.exceptions import NotFittedError

import seaborn as sns
from sklearn.linear_model import LinearRegression

from transformers import LogitsProcessorList, LogitsProcessor, AutoTokenizer, AutoModelForCausalLM


from collections import defaultdict

import os
import sys

from tqdm import tqdm, trange

from scipy.stats import hmean

from mCLM_tokenizer.tokenizer import join_fragments

device = 'cpu' #torch.device("cuda" if torch.cuda.is_available() else "cpu")

from get_LLM_output import get_molecule, get_score

# Add the src directory to sys.path
sys.path.append(os.path.abspath('../admet_oracle_model/src'))

from admet_oracle_model.src.main import prepare_dataset, evaluate, MLP, evaluate_computed

tasks = ['ames', 'bbbp', 'cyp3a4', 'dili', 'hia', 'pgp']

task_type = {
    'ames':'min',
    'bbbp':'max',
    'cyp3a4':'min',
    'dili':'min',
    'hia':'max',
    'pgp':'min',
}

ckpt_path = '../admet_oracle_model/checkpoints'

oracle_models = {}

for task in tasks:
    task_ckpt_path = os.path.join(ckpt_path, f'{task}_mlp.pt')

    oracle_models[task] = MLP().to(device)

    oracle_models[task].load_state_dict(torch.load(task_ckpt_path))



# Load block to tensor index mapping
map_info = pd.read_csv("map_info.txt")
block_to_idx = dict(zip(map_info['block'], map_info['tensor_idx']))

def select_blocks(map_info, n_caps=200, n_mids=100, seed=42):
    random.seed(seed)
    blocks = map_info['block'].tolist()

    cap_blocks = [b for b in blocks if '[3*]' in b and '[1*]' not in b and '[2*]' not in b]
    mid_blocks = [b for b in blocks if '[1*]' in b and '[2*]' in b]

    selected_caps = random.sample(cap_blocks, min(n_caps, len(cap_blocks)))
    selected_mids = random.sample(mid_blocks, min(n_mids, len(mid_blocks)))

    return selected_caps, selected_mids

valid_caps, valid_mids = select_blocks(map_info)
print(f"Selected {len(valid_caps)} caps and {len(valid_mids)} mids.")

# Load precomputed embeddings
embeddings = torch.load("precomputed_tokens.pt")

def get_embedding(smiles):
    idx = block_to_idx.get(smiles)
    if idx is None:
        raise ValueError(f"Block not found in map_info: {smiles}")
    return embeddings[idx].numpy()


def oracle_score(smiles):
    #preds = evaluate_computed(smiles) #actually a list of smiles
    preds = defaultdict(list)

    for task in tasks:
        model = oracle_models[task]

        dataloader, smiles = prepare_dataset(smiles, ckpt_path)

        all_preds = evaluate(model, dataloader, device).squeeze()

        if task_type[task] == 'min':
            all_preds = 1 - all_preds

        preds[task] = all_preds

    keys = list(preds.keys())
    for i, smi in enumerate(smiles):
        preds['hmean'].append(hmean([preds[key][i] for key in keys]))


    return preds


templates = {
    'ames':'Mutagenicity is the ability of a substance to cause genetic mutations. Drugs that harm DNA can cause serious side effects or cell death. The Ames test, created by Professor Ames, is a widely used method for testing mutagenicity. It is a bacterial reverse mutation test that identifies compounds that cause genetic alterations and frameshift mutations. Does the compound with the SMILES string [MOL][/MOL] show mutagenic potential in the Ames test?',
    'bbbp':'The blood-brain barrier (BBB) is a protective membrane that separates blood from brain extracellular fluid, effectively blocking the majority of drugs. This limitation is a significant hurdle for creating effective therapies for the central nervous system. Can [MOL][/MOL] penetrate the blood-brain barrier?',
    'cyp3a4':'CYP P450 genes are involved in the formation and metabolism of various compounds within cells. CYP3A4 is a crucial enzyme primarily found in the liver and intestines, where it oxidizes foreign organic substances like toxins and drugs, enabling their excretion from the body. Explore whether the drug [MOL][/MOL] can be considered a substrate to CYP3A4.',
    'dili':'DILI is a serious liver disease caused by drugs, and for over 50 years, it has been the leading cause of safety-related drug market withdrawals (e.g., iproniazid, ticrynafen, benoxaprofen). Is the drug with the SMILES notation [MOL][/MOL] associated with liver injury?',
    'hia':'Human intestinal absorption (HIA) is the process by which an orally taken drug enters the bloodstream from the gastrointestinal system, playing a crucial role in its effectiveness. Can the drug [MOL][/MOL] be absorbed in the human intestine?',
    'pgp':'P-glycoprotein (Pgp) is responsible for drug transport across biological barriers, influencing absorption, metabolism, and brain penetration, and its inhibition can lead to significant changes in drug bioavailability. Notably, blocking Pgp activity can be a strategy to fight multidrug resistance. I want to assess if the compound [MOL][/MOL] interacts with Pgp.',
}
mol_templates ={
    'ames':'I am looking for a molecule with no mutagenicity (Ames test).',
    'bbbp':'I am looking for a molecule with good blood-brain barrier permeability.',
    'cyp3a4':'I am looking for a molecule with CYP3A4 inhibitory activity.',
    'dili':'I am looking for a molecule that does not cause drug-induced liver injury.',
    'hia':'I need a molecule with good Human Intestinal Absorption.',
    'pgp':'I am looking for a molecule with PGP inhibition.',

}



molecule_size = 3



# Simulated LLM oracle
def simulated_llm_property(mols):
    
    preds = defaultdict(list)

    for task in tqdm(tasks):
        all_preds = []

        for s, m, e, _, _ in tqdm(mols):
            score = get_score(templates[task].replace('[MOL][/MOL]','[MOL]'+'^'.join([s,m,e])+'[/MOL]'))
            all_preds.append(score)

        all_preds = np.array(all_preds)

        if task_type[task] == 'min':
            all_preds = 1 - all_preds

        preds[task] = all_preds

    keys = list(preds.keys())
    for i, _ in enumerate(mols):
        preds['hmean'].append(hmean([preds[key][i] for key in keys]))


    return preds



    
# Embed triple (start, mid, end) into vector
def get_molecule_embedding(start, mid, end):
    try:
        s_emb = get_embedding(start)
        m_emb = get_embedding(mid)
        e_emb = get_embedding(end)
        return np.concatenate([s_emb, m_emb, e_emb])
    except Exception as e:
        print(f"[embedding error] {e}")
        return None

# Find nearest fragment by cosine similarity
def find_nearest_fragment(target_emb, fragment_list, is_mid=False):
    best_sim = -1
    best_frag = None
    for frag in fragment_list:
        try:
            frag_emb = get_embedding(frag)
            sim = np.dot(target_emb, frag_emb) / (np.linalg.norm(target_emb) * np.linalg.norm(frag_emb))
            if sim > best_sim:
                best_sim = sim
                best_frag = frag
        except:
            continue
    return best_frag

# Visualization of active learning loop
def plot_llm_vs_oracle(llm_r2_history, llm_mse_history):
    rounds = list(range(1, len(llm_r2_history)+1))
    fig, ax1 = plt.subplots(figsize=(8,5))

    ax1.set_xlabel("Round")
    ax1.set_ylabel("LLM R²", color='tab:blue')
    ax1.plot(rounds, llm_r2_history, marker='o', color='tab:blue', label="LLM R²")
    ax1.tick_params(axis='y', labelcolor='tab:blue')

    ax2 = ax1.twinx()
    ax2.set_ylabel("LLM MSE", color='tab:red')
    ax2.plot(rounds, llm_mse_history, marker='s', linestyle='--', color='tab:red', label="LLM MSE")
    ax2.tick_params(axis='y', labelcolor='tab:red')

    plt.title("LLM Prediction vs Oracle (per Round)")
    fig.tight_layout()
    plt.savefig('results_LLM_gen.png')
    
def plot_scatter_with_trend(llm_values, oracle_values):
    # Convert to numpy arrays for sklearn
    x = np.array(llm_values).reshape(-1, 1)
    y = np.array(oracle_values)

    # Fit linear regression
    model = LinearRegression()
    model.fit(x, y)
    y_pred = model.predict(x)
    r2 = r2_score(y, y_pred)

    # Plot
    plt.figure(figsize=(8, 6))
    sns.scatterplot(x=llm_values, y=oracle_values, label='Data', color='blue')
    sns.lineplot(x=llm_values, y=y_pred, color='red', label='Trend line')
    plt.xlabel("LLM Value")
    plt.ylabel("Oracle Value")
    plt.title(f"Scatter Plot with Trend Line (R² = {r2:.2f})")
    plt.legend()
    plt.tight_layout()
    plt.savefig('oracle_vs_llm_gen.png')

def plot_cum_max_value(cum_max):
    rounds = list(range(1, len(cum_max)+1))
    fig = plt.figure()

    plt.xlabel("Round")
    plt.plot(rounds, cum_max, marker='o', color='tab:blue', label='Oracle')
    #plt.plot(rounds, llm_cum_max, marker='s', color='tab:orange', label='LLM')

    plt.title("Max Cumulative Score vs Oracle (per Round)")
    plt.legend()
    plt.tight_layout()
    plt.savefig('cum_max_value_LLM_gen.png')



class SurrogateNN(nn.Module):
    def __init__(self, input_dim):
        super().__init__()
        self.model = nn.Sequential(
            nn.Linear(input_dim, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 1)
        )

    def forward(self, x):
        return self.model(x)


class ActiveLearner:
    def __init__(self, caps, mids, embedding_dim):
        self.caps = caps
        self.mids = mids
        self.X = []
        self.y = []

        self.scaler = StandardScaler()
        self.model = SurrogateNN(input_dim=embedding_dim * 3)
        self.optimizer = optim.Adam(self.model.parameters(), lr=1e-3)
        self.criterion = nn.MSELoss()


    def add_observation(self, emb, prop):
        self.X.append(emb)
        self.y.append(prop)

    def train(self):
        if len(self.X) < 10:
            return
        X_array = np.array(self.X)
        y_array = np.array(self.y).reshape(-1, 1)

        if len(self.X) > 10:
            max_components = min(len(self.X), X_array.shape[1])

        X_scaled = self.scaler.fit_transform(X_array)

        X_tensor = torch.tensor(X_scaled, dtype=torch.float32)
        y_tensor = torch.tensor(y_array, dtype=torch.float32)

        self.model.train()
        for _ in range(200):  # epochs
            self.optimizer.zero_grad()
            y_pred = self.model(X_tensor)
            loss = self.criterion(y_pred, y_tensor)
            loss.backward()
            self.optimizer.step()

    def predict(self, emb):
        self.model.eval()
        try:
            emb_scaled = self.scaler.transform([emb])
        except NotFittedError:
            print("[predict] Skipping prediction: Scaler not fitted.")
            return None
    
        emb_tensor = torch.tensor(emb_scaled, dtype=torch.float32)
        with torch.no_grad():
            return self.model(emb_tensor).item()


    def suggest_top_uncertain(self, blocks, n=100):
        suggestions = []
        #for _ in range(300):
        for s, m, e in blocks:
            #s = random.choice(self.caps)
            #m = random.choice(self.mids)
            #e = random.choice(self.caps)
            emb = get_molecule_embedding(s, m, e)
            if emb is None:
                continue
            
            # Simulate uncertainty by sampling predictions multiple times
            pred_samples = []
            for _ in range(10):  # Number of Monte Carlo samples for uncertainty estimation
                pred = self.predict(emb)
                if pred is not None:
                    pred_samples.append(pred)
            
            if len(pred_samples) < 5:
                continue
    
            # Calculate variance of the predictions
            pred_mean = np.mean(pred_samples)
            pred_variance = np.var(pred_samples)
            
            # Use the variance as a proxy for uncertainty
            suggestions.append((s, m, e, pred_mean, pred_variance))  # Higher variance = more uncertain
        
        def acquisition(mu, sigma):
            return mu + 1.96 * sigma # UCB acquisition

        # Sort by uncertainty (variance) and select the top `n` uncertain molecules
        sorted_suggestions = sorted(suggestions, key=lambda x: acquisition(x[3], x[4]), reverse=True)
        
        return sorted_suggestions[:n]

    def get_metrics(self):
        if not self.X:
            return None, None
        X_array = np.array(self.X)
        y_array = np.array(self.y)

        X_scaled = self.scaler.transform(X_array)
        X_tensor = torch.tensor(X_scaled, dtype=torch.float32)

        self.model.eval()
        with torch.no_grad():
            y_pred = self.model(X_tensor).numpy().flatten()
        return r2_score(y_array, y_pred), mean_squared_error(y_array, y_pred)

# Main active learning loop
def active_learning_loop(n_rounds=10):
    learner = ActiveLearner(valid_caps, valid_mids, embedding_dim=embeddings[0].shape[0])
    all_results = []

    banned_molecules = []

    # Zero-shot: no initial training, start from suggestions
    print("[zero-shot] Starting cold...")
    mols = []
    valid_blocks = valid_caps + valid_mids
    total_blocks = []
    for task in tasks:
        blocks, block_ids, _ = get_molecule(mol_templates[task], allowed_token_ids=None, allowed_blocks=valid_blocks, num_mols=2, bad_word_ids=banned_molecules)
        total_blocks = total_blocks + blocks
        banned_molecules = banned_molecules + block_ids

        #print(total_blocks)
        #print(banned_molecules)

    for s, m, e in total_blocks: #while len(learner.X) < 10: # Random suggestions for starting
        
        #s = random.choice(valid_caps)
        #m = random.choice(valid_mids)
        #e = random.choice(valid_caps)
        mol = join_fragments(f'{s}^{m}^{e}')
        if mol is None:
            continue
        emb = get_molecule_embedding(s, m, e)
        if emb is None:
            continue
        mols.append((s, m, e, Chem.MolToSmiles(mol), emb))

    oracle = oracle_score([smi for s, m, e, smi, _ in mols])
    #preds = simulated_llm_property(mols)

    #print([smi for s, m, e, smi, _ in mols])

    for (s, m, e, smi, emb), o in zip(mols, oracle['hmean']):
        learner.add_observation(emb, o)
        all_results.append((s, m, e, Chem.MolToSmiles(mol), np.nan, o))
    
    learner.train()
    llm_r2_history = []
    llm_mse_history = []
    cumulative_max = []
    LLM_cumulative_max = []
    r2, mse = learner.get_metrics()
    llm_preds = [entry[4] for entry in all_results]     # simulated LLM values
    oracle_vals = [entry[5] for entry in all_results]   # true LogP values
    
    cumulative_max.append(max(oracle_vals))
    #LLM_cumulative_max.append(max(llm_preds))

    #llm_r2 = r2_score(oracle_vals, llm_preds)
    #llm_mse = mean_squared_error(oracle_vals, llm_preds)
    #llm_r2_history.append(llm_r2)
    #llm_mse_history.append(llm_mse)
    print(f" Model R²: {r2:.3f}, MSE: {mse:.3f} | Cumulative Max: {max(oracle_vals):.3f} | LLM Cumulative Max: {max(llm_preds):.3f}")
    
    for r in trange(n_rounds-1):
        print(f"\n[Round {r+1}] Suggesting molecules...")

        valid_blocks = valid_caps + valid_mids
        total_blocks = []
        for task in tqdm(tasks):
            blocks, block_ids, _ = get_molecule(mol_templates[task], allowed_token_ids=None, allowed_blocks=valid_blocks, num_mols=15, bad_word_ids=banned_molecules)
            total_blocks = total_blocks + blocks
            banned_molecules = banned_molecules + block_ids
            
        selected = learner.suggest_top_uncertain(total_blocks, 10)
        if len(selected) < 10:
            print("[!] Not enough suggestions; skipping.")
            break
        #selected = random.sample(suggestions, 10)  # Simulate LLM selection
        

        mols = []
        for s, m, e, _, _ in selected:
            mol = join_fragments(f'{s}^{m}^{e}')
            if mol is None:
                continue
            emb = get_molecule_embedding(s, m, e)
            if emb is None:
                continue
            mols.append((s, m, e, Chem.MolToSmiles(mol), emb))
        
        #pred_selected = simulated_llm_property(mols)

        #top_indices = np.argsort(pred_selected['hmean'])[-10:][::-1]
        #selected = [mols[i] for i in top_indices]
        #print([smi for s, m, e, smi, _ in mols])

        oracle = oracle_score([smi for s, m, e, smi, _ in mols])
        #preds = simulated_llm_property(selected)
        
        for (s, m, e, smi, emb), o in zip(mols, oracle['hmean']):
            learner.add_observation(emb, o)
            all_results.append((s, m, e, smi, np.nan, o))
            

        learner.train()
        r2, mse = learner.get_metrics()
        #llm_preds = [entry[4] for entry in all_results]     # simulated LLM values
        oracle_vals = [entry[5] for entry in all_results]   # true LogP values

        #llm_r2 = r2_score(oracle_vals, llm_preds)
        #llm_mse = mean_squared_error(oracle_vals, llm_preds)
        #llm_r2_history.append(llm_r2)
        #llm_mse_history.append(llm_mse)
        cumulative_max.append(max(oracle_vals))
        #LLM_cumulative_max.append(max(llm_preds))
        
        print(f" Model R²: {r2:.3f}, MSE: {mse:.3f} | Cumulative Max: {max(oracle_vals):.3f} | LLM Cumulative Max: {max(llm_preds):.3f}")

    #plot_llm_vs_oracle(llm_r2_history, llm_mse_history)
    plot_cum_max_value(cumulative_max)

    
    #llm_preds = [entry[4] for entry in all_results]     # simulated LLM values
    oracle_vals = [entry[5] for entry in all_results]   # true LogP values
    
    #plot_scatter_with_trend(llm_preds, oracle_vals)

    return all_results

# Run
results = active_learning_loop(n_rounds=10)



print(results)



