...
if __name__ == "__main__":
    ...
    #uncomment me for OOD number test
    ...
    parser.add_argument("--tokenizer_path", default="./Top1k_FDA2/", type=str)
    ...
    parser.add_argument("--pretrained_embeddings", default="/home/cne2/data/Chemistry/mCLM_MolCLR/preprocess/Top500V2/FDA/", type=str)
    ...
    tasks = ['hia']#['ames', 'bbbp', 'cyp3a4', 'dili', 'hia', 'pgp']
    ...
    df['blocks'] = df['blocks'].apply(lambda x : x.split('^'))
    ...
    namedf = pd.read_csv('drugs_processed.csv')
    namedf['SMILES'] = namedf['SMILES'].apply(canonicalize)

    namedf.set_index('SMILES', inplace=True)


    print(df)

    smiles = df['SMILES'].tolist()
    namedf_unique = namedf[~namedf.index.duplicated(keep='first')]
    names = namedf_unique.reindex(smiles)['Name'].to_list()
    ...
    print('FDA blocks:', len(new_blocks))
    all_synth_blocks = set([s.strip() for s in open('/shared/nas/data/m1/shared-resource/MoleculeLanguage/mCLM/synth_blocks_top5000000k.txt').readlines()])

    synth_keys = []
    for key in molecule_tokenizer.idx_to_block:
        blck = molecule_tokenizer.idx_to_block[key]
        if blck in all_synth_blocks or blck in new_blocks:
            synth_keys.append(key)

    print('synth_keys', len(synth_keys))

    id2b = molecule_tokenizer.idx_to_block
    #union_blocks = new_blocks.union(set([id2b[k] for k in id2b]))

    # Write to file
    with open("synth_block_vocab.txt", "w") as f:
        for block in sorted([id2b[key] for key in synth_keys]):
            f.write(str(block) + "\n")

    zz

    if False: #to use this, set the tokenizer_path Top50V2/Qwen2.5-3B_TotalTop1k_splitLoss_1kPretrain
    ...
    for task in tqdm(tasks):
        ...
        for smi, blck in tqdm(zip(smiles, blocks), total=len(blocks)):
            ...
            token_ids, mols = get_molecule(inst, resp, allowed_token_ids=synth_keys)
    ...
    os.makedirs(f'images/{tasks[0]}/', exist_ok=True)

    for i, (name, smi) in enumerate(zip(names, smiles)):
        name = str(name).replace('<n>', '').replace('</n>', '').replace('/', '').replace('<', '').replace('>', '')
