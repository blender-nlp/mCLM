...
parser.add_argument("--out_tokenizer_path", default="./Top1k_FDA/", type=str)
...
df = pd.read_csv('DrungBank_FDA_Approved.csv')
df = df[df['SMILES'].notnull()]

df = df[df['SMILES'].str.len() <= 512]


#print(df)
smiles_list = df['SMILES'].to_list()

#scaffold_list = df['SMILES'].apply(get_scaffold).to_list()
#df['scaffolds'] = scaffold_list

new_mols = set()

mol_list = smiles_list# + scaffold_list
...
for mols in tqdm(smiles_list):
    blks = get_blocks(mols)
    if len (blks) > 0 and is_blocks(blks[0]):
            block_list.append(blks[0].split('.'))
    else: 
        #print('Problem processing:', mols) #note to self, throw out temoporfin in eval. 
        block_list.append([])
        unproced += 1

#for mols in tqdm(scaffold_list):
#    blks = get_blocks(mols)
#    if len (blks) > 0 and is_blocks(blks[0]):
#            scaff_block_list.append(blks[0].split('.'))
#    else: 
#        #print('Problem processing:', mols) #note to self, throw out temoporfin in eval. 
#        scaff_block_list.append([])
#        unproced += 1

print(len(smiles_list) -unproced, '/', len(mol_list), 'were able to be tokenized.')

df['blocks'] = block_list
#df['scaff_blocks'] = scaff_block_list
...
df.to_csv('drugs_processed.csv', index=False)
