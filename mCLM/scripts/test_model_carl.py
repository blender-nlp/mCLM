...
def main(args):
    ...
    core_model.finalize_molecule_embeddings(
        embeddings=nn.Embedding(core_model.mol_vocab_size,
                                core_model.model.config.molecule_config["input_dim_adapter"]
                               ).to(core_model.device).to(torch.bfloat16))

    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained(dm.config['pretrained_tokenizer'])
