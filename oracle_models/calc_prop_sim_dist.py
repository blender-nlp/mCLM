...
# Load the dataset
df = pd.read_csv("data.csv", index_col='molecule')

sims = pd.read_parquet("tanimoto_similarity_matrix.parquet")


props = pd.Series(list((set(df.keys()) - set(['molecule']))))
...
for p in bar:
    ...
    for pmol in pos_mols:
        mol_sims = sims[pmol]

        sim_mols = mol_sims[mol_sims >= thresh].index
        sim_mols = [sm for sm in sim_mols if sm not in pos_mols]
...
plt.savefig('prop_sim_histo.png')
